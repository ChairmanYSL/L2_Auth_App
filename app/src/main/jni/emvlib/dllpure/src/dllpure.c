...
unsigned char pure_Outcome_CommunicationErrors_PaymentTransactionNotCompleted(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_Trace("Start pure_Outcome_CommunicationErrors_PaymentTransactionNotCompleted\r\n");
...
unsigned char pure_Outcome_TransactionCompletedOnError(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SendOutcome();
	EMVBase_Trace("Start pure_Outcome_TransactionCompletedOnError\r\n");
...
unsigned char pure_Outcome_CommunicationErrors_UnknownPaymentResult(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_Trace("Start pure_Outcome_CommunicationErrors_UnknownPaymentResult\r\n");
	return RLT_ERR_EMV_TransRestart;
}
...
unsigned char pure_Outcome_FinancialTransactionCompleted_OndeviceCVMRequested(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SendOutcome();
	sdkmSleep(gOutcomeDelay);
	tempApp_UnionStruct->SetUIRequest(SDK_UI_MSGID_SEEPHONE, SDK_UI_STATUS_PROCESSINGERR, 0x13, NULL, SDK_UI_VALUEQUALIFIER_NA, NULL, NULL);
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_OUTCOME);
	sdkmSleep(gOutcomeDelay);
	tempApp_UnionStruct->SetUIRequest(SDK_UI_MSGID_TRYAGAIN, SDK_UI_STATUS_READYTOREAD, 0, NULL, SDK_UI_VALUEQUALIFIER_NA, NULL, NULL);
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_RESTART);

	EMVBase_Trace("Start pure_Outcome_FinancialTransactionCompleted_OndeviceCVMRequested\r\n");
...
unsigned char pure_Outcome_FinancialTransactionCompleted_TryOtherInterface(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_Trace("Start pure_Outcome_FinancialTransactionCompleted_TryOtherInterface\r\n");
...
unsigned char pure_Outcome_FinancialTransactionCompleted_TransactionApproved(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SetOutcome(SDK_OUTCOME_RESULT_APPROVED, SDK_OUTCOME_START_NA, CVM, 1, 0, 1, 1, SDK_OUTCOME_AIP_NA, 0, SDK_OUTCOME_FIELDOFFREQ_NA, NULL, SDK_OUTCOME_ONLINERESPDATA_NA);
	tempApp_UnionStruct->SendOutcome();
	sdkmSleep(gOutcomeDelay);
	...
	EMVBase_Trace("Start pure_Outcome_FinancialTransactionCompleted_TransactionApproved\r\n");
	return RLT_EMV_OFFLINE_APPROVE;
}

unsigned char pure_Outcome_FinancialTransactionCompleted_TransactionDeclined(PURETradeUnionStruct *tempApp_UnionStruct, unsigned char dataRcdPresent)
{
	tempApp_UnionStruct->SetOutcome(SDK_OUTCOME_RESULT_DECLINED, SDK_OUTCOME_START_NA, SDK_OUTCOME_CVM_NA, 1, 0, dataRcdPresent, 1, SDK_OUTCOME_AIP_NA, 0, SDK_OUTCOME_FIELDOFFREQ_NA, NULL, SDK_OUTCOME_ONLINERESPDATA_NA);
	tempApp_UnionStruct->SendOutcome();
	sdkmSleep(gOutcomeDelay);
	...
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_OUTCOME);
	EMVBase_Trace("Start pure_Outcome_FinancialTransactionCompleted_TransactionDeclined\r\n");
...
{
	unsigned char CVM = tempApp_UnionStruct->EMVTradeParam->PureCVMParameter;
	unsigned char RemovalTimeout[2]={0};
...
unsigned char pure_Outcome_FinancialTransactionCompleted_OnlineRequestNoAdditionalTap(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SetOutcome(SDK_OUTCOME_RESULT_ONLINE_NOADDPREMENT, SDK_OUTCOME_START_NA, CVM, 1, 0, 1, 1, SDK_OUTCOME_AIP_NA, 0, SDK_OUTCOME_FIELDOFFREQ_NA, NULL, SDK_OUTCOME_ONLINERESPDATA_NA);
	tempApp_UnionStruct->SendOutcome();
	sdkmSleep(gOutcomeDelay);

	EMVBase_Trace("CVM = %02X\r\n", CVM);
	if(SDK_OUTCOME_CVM_ONLINEPIN == CVM)
	{
		tempApp_UnionStruct->SetUIRequest(SDK_UI_MSGID_PLSENTERPIN, SDK_UI_STATUS_CARDREADSUCCESS, 0, NULL, SDK_UI_VALUEQUALIFIER_BALANCE, NULL, NULL);
	}
...
unsigned char pure_Outcome_NonFinancialCompleted_OfflineAuthentication(PURETradeUnionStruct * tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_OUTCOME);
	EMVBase_Trace("Start pure_Outcome_NonFinancialCompleted_OfflineAuthentication\r\n");
...
unsigned char pure_Outcome_NonFinancialCompleted_RetrievalOfDataElements(PURETradeUnionStruct * tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SetUIRequest(SDK_UI_MSGID_PLSREMOVECARD, SDK_UI_STATUS_CARDREADSUCCESS, 0, NULL, SDK_UI_VALUEQUALIFIER_NA, NULL, NULL);
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_OUTCOME);
	EMVBase_Trace("Start pure_Outcome_NonFinancialCompleted_RetrievalOfDataElements\r\n");
	return RLT_EMV_TERMINATE_TRANSERR;
}
...
unsigned char pure_Outcome_EndApplication_EmptyCandidateList(PURETradeUnionStruct * tempApp_UnionStruct)
{
	...
	tempApp_UnionStruct->SendUIRequest(PURE_UIREQ_OUTCOME);
	EMVBase_Trace("Start pure_Outcome_EndApplication_EmptyCandidateList\r\n");
	return RLT_EMV_TERMINATE_TRANSERR;
}
...
{
    unsigned char *IPKData, IPKHash[20]={0};
    unsigned char buf[9]={0}, bytePAN[8]={0}, byteIssuID[8]={0};
...
//            return RLT_ERR_EMV_IccDataMissing;
	...
	EMVBase_TraceHex("Issuer PK Moduls", tempApp_UnionStruct->EMVTradeParam->IPKModul, tempApp_UnionStruct->EMVTradeParam->IPKModulLen);
    if(IPKRem)
    {
        emvbase_free(IPKRem);
    }
...
{
    unsigned char *ICCPKData, ICCPKHash[20]={0};
    unsigned short index;
    EMVBASE_ICCPK_RECOVER recovICCPK;
    EMVBASETAGCVLITEM *item, *item1, *item_taglist, *item_tag;
    unsigned char *ICCPKToSign;
    unsigned char testlen;
    unsigned char tmppan1[20], tmppan1len, tmppan2[20]={0}, tmppan2len;
	...
	if(tempApp_UnionStruct->EMVTradeParam->AuthDataLen)
	{
		...
		EMVBase_TraceHex("pure-info: static data", tempApp_UnionStruct->EMVTradeParam->AuthData, tempApp_UnionStruct->EMVTradeParam->AuthDataLen);
	}
	...
	if(emvbase_avl_checkiftagexist(EMVTAG_SDATagList))
	{
		item_taglist = emvbase_avl_gettagitempointer(EMVTAG_SDATagList);
		if(item_taglist->len == 1)	//9F4A contain only 82
		{
			memcpy(Tag, item_taglist->data, item_taglist->len);
			item_tag = emvbase_avl_gettagitempointer(Tag);
			memcpy((unsigned char*)&ICCPKToSign[index], item_tag->data, item_tag->len);
			index += item_tag->len;
		}
	}

	EMVBase_TraceHex("pure-info: recover ICCPK input hash data", ICCPKToSign, index);

//    EMVBase_Hash(ICCPKToSign, index, ICCPKHash);
//	EMVBase_TraceHex("PURE-info: calculate ICCPKHash", ICCPKHash, 20);

	memset(ICCPKHash, 0, sizeof(ICCPKHash));
    ...
    if(recovICCPK.HashInd == 0x01)  //SHA-1 algorithm
    {
        ...
        {
        	EMVBase_Trace("PURE-info: Hash Result verification fails\r\n");
			EMVBase_TraceHex("PURE-info: ICCPKHash", ICCPKHash, 20);
			EMVBase_TraceHex("PURE-info: recovICCPK.HashResult", recovICCPK.HashResult, 20);
            if(ICCPKRem)
            {
                emvbase_free(ICCPKRem);
            }
...
unsigned char pure_CompareIACTAC(PURETradeUnionStruct *tempApp_UnionStruct)
{
    ...
    unsigned char TACDenial[5]={0};
    unsigned char IACDenial[5]={0}, IACDenialbExist;
    unsigned char TermType;
    unsigned char TACOnline[5]={0}, IACOnline[5]={0}, IACOnlinebExist;
    unsigned char TACDefault[5]={0}, IACDefault[5]={0}, IACDefaultbExist;
    unsigned char bExist;
	unsigned char TTPI[5]={0};
...
//	}
	...
	if((TermType == 1 || TermType == 2 || TermType == 4 || TermType == 5) && ((TTPI[0] & 0x08) == 0)) //FIXME: Terminal has Online capability
    {
...
unsigned char pure_ParseAndStoreCardResponse(PURETradeUnionStruct *tempApp_UnionStruct, unsigned char *DataOut, unsigned short *DataOutIndex, unsigned char *parsebInTable, unsigned char readstep)
{
    ...
    unsigned char tag[4]={0}, taglen,tagItemLen;
    unsigned char ret;
...
unsigned char pure_CheckGACRespMandatoryDataMiss(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_Trace("pure-info: OfflineCAMSelectedIndicator: %02X\r\n\r\n", tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator);

	if(0x02 != tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator)	//CDA not requested
	{
	...
	if(CCIDExist)	//AAC card decision with CDA requested
	{
		if((0x00 == (CID & 0xC0)) && (0x00 == (CCID & 0xC0)) && (0x02 == tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator))
		{
			...
			{
				EMVBase_Trace("pure-error: miss 9F26 and 9F4B\r\n");
...
unsigned char pure_PDOLProcess(PURETradeUnionStruct *tempApp_UnionStruct, EMVBASE_APDU_SEND *apdu_s)
{
    ...
    if(item != NULL)
    {
        ...
        {
            PDOLbexist = 1;
			EMVBase_Trace("pure-info: PDOL len = %d\r\n", item->len);
        }
    }
    ...
    while(index < tempPDOLlen)  		//Process PDOL
    {
		...
		EMVBase_TraceHex("pure-info: PDOL tag", tag, 3);

		if((memcmp(tag, "\x9F\x50", 2) < 0) || (memcmp(tag, "\x9F\x7F", 2) > 0) || (!memcmp(tag, "\x9F\x7A", 2)) || (!memcmp(tag, "\x9F\x76", 2)))
		{
        	pdoldataitem = emvbase_avl_gettagitempointer(tag);
			EMVBase_Trace("pure-info: pdoldataitem: %p", pdoldataitem);
        	if(pdoldataitem != NULL)
	        {
	            ...
	            {
					if(EMVBase_ParseSingleByteLen(PDOL, &index, &templen))
					{
						emvbase_free(pdata);
						EMVBase_Trace("pure-info: PDOL error flag1\r\n");
						return RLT_ERR_EMV_IccDataFormat;
					}
	            }
        ...
        if(!bInTable)
        {
            ...
            {
                emvbase_free(pdata);
				EMVBase_Trace("pure-info: PDOL error flag2\r\n");
                return RLT_ERR_EMV_IccDataFormat;
            }
            k = templen;
			if ((indexOut + k) > DOLDataMaxLen)//20201217 lsy
...
unsigned char pure_DelGPORetData(PURETradeUnionStruct *tempApp_UnionStruct, EMVBASE_APDU_RESP *apdu_r)
{
	...
	unsigned char AIPTagExist = 1, AIP[2]={0};
	unsigned char retCode;
	...
	if(CCIDExist)
	{
		...
		tempApp_UnionStruct->EMVTradeParam->CCIDInGPO = CCID;
...
unsigned char pure_InitialApp(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char ImpleOption, CombOptions[2]={0};
	...
	if(retCode != RLT_EMV_OK)
	{
		...
		{
			return pure_Outcome_FinancialTransactionCompleted_TransactionDeclined(tempApp_UnionStruct, 0);
		}
		else if(RLT_ERR_EMV_IccDataMissing == retCode)
		{
			return pure_Outcome_TransactionCompletedOnError(tempApp_UnionStruct);
...
void pure_RecoverPanFromTrack2EQU(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char Track2Equ[20]={0};
	unsigned char pan[10]={0}, panlen=0;
...
unsigned char pure_initDynamicAuth(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	if(tempcapk == NULL)
	{
		EMVBase_Trace("pure-info: Read Term CAPK fail\r\n");
		return RLT_EMV_ERR;
	}

    if(tempcapk->ModulLen == 0)
    {
		EMVBase_Trace("pure-info: Term CAPK Modul Len invalid\r\n");
        return RLT_EMV_ERR;
	}

    if(tempApp_UnionStruct->EMVTradeParam->IPKModulLen == 0)
    {
		retCode = pure_GetIPK(tempApp_UnionStruct);
		EMVBase_Trace("pure-info: pure_GetIPK ret = %d\r\n", retCode);
        if(retCode != RLT_EMV_OK)  //retrieval of the issuer public key modulus
        {
            return RLT_EMV_ERR;
        }
    ...
    {
		retCode = pure_GetICCPK(tempApp_UnionStruct);
		EMVBase_Trace("pure-info: pure_GetICCPK ret = %d\r\n", retCode);
        if(retCode != RLT_EMV_OK)  //retrieval of the ICC public key
        {
            return RLT_EMV_ERR;
        }
    }
...
{
	unsigned char AIP[2]={0};
	unsigned char ODAFail = 0, ODANotPerFlag = 1;
	...
	if(((AIP[0] & 0x01) && (KernelCap[4] & 0x01)) && ((0 == (ImpleOption & 0x10)) || (PURE_TRANS_APPAUTHTRANS != TransType)))//ICC and terminal support Combined DDA/AC
	{
		...
		else
		{
			...
			{
				tempApp_UnionStruct->EMVTradeParam->ODADataChecked = 1;
...
//		if(ODANotPerFlag)
//		{
//			EMVBase_Trace("PURE-info: set TVR byte1 bit8 = 1\r\n");
//			emvbase_avl_setvalue_or(EMVTAG_TVR, 0, 0x80);		//ODA not performed
//		}
	}
...
void pure_RandTransSelect(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char FloorLimit[6]={0};
	unsigned char Threshold[6]={0};
	...
	unsigned char  TVR[5]={0};
	unsigned char TermFloorLmtHex[4]={0};
	unsigned short tmp;
...
int pure_IccGetPAN(unsigned char *pasOutPAN)
{
    ...
    unsigned char Track2Len, Track2Data[20]={0};
...
unsigned char pure_CheckVelocity(unsigned char bATCExist,unsigned char bLOATCExist)
{
	...
	unsigned char LOATC[2]={0},ATC[2]={0};
...
{
	int retCode;
	unsigned char AIP[2]={0}, KernelCap[5]={0};
...
{
    unsigned char currentDate[4]={0}, EffectDate[4]={0}, ExpireDate[4]={0};   //buf[10],
...
void pure_CheckAVN(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char AVNCard[2]={0},AVNTerm[2]={0};
...
void pure_CheckAUC(PURETradeUnionStruct *tempApp_UnionStruct)
{
    ...
    unsigned char TermType,AUC[2]={0},AUCbExist;
    unsigned char IssuCountryCode[2]={0},CountryCode[2]={0},IssuCountryCodebExist;
    unsigned char TermAmtOther[6]={0};
    unsigned char TermAddCapab[5]={0};
...
static int pure_GetPAN(PURETradeUnionStruct *tempApp_UnionStruct, unsigned char *pasOutPAN)
{
    unsigned char temp[128] = {0};
    unsigned char bcPAN[128] = {0};
    unsigned int i, j, len;
	...
	{
		pure_RecoverPanFromTrack2EQU(tempApp_UnionStruct);
		return pure_GetPAN(tempApp_UnionStruct, pasOutPAN);
	}
#else
    if(tagexistflag)
    {
...
unsigned char pure_ProcessRestrict(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char SZZTICS[7]={0};
	unsigned char asPAN[19] = {0};
	int ret;
	...
	ret = pure_GetPAN(tempApp_UnionStruct, asPAN);
	EMVBase_Trace("pure-info: pure_GetPAN ret = %d\r\n", ret);
	EMVBase_Trace("pure-info: asPAN = %s", asPAN);

	EMVBase_Trace("pure-info: (For Debug Only)tempApp_UnionStruct->EMVTradeParam->CAPK: %p\r\n", tempApp_UnionStruct->EMVTradeParam->CAPK);

	if(ret == RLT_EMV_OK)
	{
		EMVBase_Trace("pure-info: VerifyCardNo addr = %p\r\n", tempApp_UnionStruct->VerifyCardNo);
		if(tempApp_UnionStruct->VerifyCardNo)
		{
			ret = tempApp_UnionStruct->VerifyCardNo(asPAN);
			EMVBase_Trace("pure-info: VerifyCardNo ret = %d\r\n", ret);
			if(ret != 0)
			{
				EMVBase_Trace("pure-info: PAN in Exception File\r\n");
				emvbase_avl_setvalue_or(EMVTAG_TVR, 0, 0x10);
			}
		}
	}
...
unsigned char pure_CommandGAC1(PURETradeUnionStruct *tempApp_UnionStruct, unsigned char TermAnaResult, EMVBASE_APDU_SEND *apdu_s)
{
	...
	if(0x02 == tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator)
	{
		...
		EMVBase_Trace("PURE-info: TermAnaResult: %02X\r\n", TermAnaResult);
		EMVBase_Trace("PURE-info: TTPI Byte 2: %02X\r\n", TTPI[1]);
		EMVBase_Trace("PURE-info: TVR Byte 1: %02X\r\n", TVR[0]);
	...
	EMVBase_Trace("PURE-info: after pack GAC command OfflineCAMSelectedIndicator: %02X\r\n", tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator);
...
unsigned char pure_DealGAC1Rsp(PURETradeUnionStruct *tempApp_UnionStruct, EMVBASE_APDU_RESP *apdu_r, unsigned char TermAnaResult)
{
    unsigned char ret;
	unsigned short index,indexTemp,lenTemp,orgindex,indexRet;
	unsigned char bIntable;
	...
	CCIDInGPOExist = tempApp_UnionStruct->EMVTradeParam->CCIDExistInGPO;
	if(CCIDInGPOExist)
	{
	...
	else
	{
		if(1 == CCIDInGPOExist && 1 == CCIDInGACExist)
		{
			...
			if((tempApp_UnionStruct->EMVTradeParam->CCIDInGPO & 0x07) != (CCIDInGAC & 0x07))	//CVM Information (3 least significant bits) has a distinct value in both CCID values
			{
...
{
    unsigned char *SDAToSign, SDAHash[20]={0}, *SDAData;    //[512]   [248]
    unsigned int index;
    ...
    unsigned char AmtAuthNum[6]={0};
    unsigned char TransCurcyCode[2]={0};
    unsigned char DDAVersionType = 0;
    unsigned char CardUnpredictNum[20]={0};
...
unsigned char pure_CombineDDASignVerify(unsigned char type, PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_TraceHex("PURE-info: ICC PK Modul", tempApp_UnionStruct->EMVTradeParam->ICCPKModul, tempApp_UnionStruct->EMVTradeParam->ICCPKModulLen);

	EMVBase_TraceHex("PURE-info: input hash data", SDAToSign, index);

    EMVBase_Hash_Back(SDAToSign, index, SDAHash);

	EMVBase_TraceHex("PURE-info: recovSDA", (u8 *)recovSDA, sizeof(EMVBASE_SIGN_DYNDATA_RECOVER));

	EMVBase_TraceHex("PURE-info: recovSDA->HashResult", recovSDA->HashResult, 20);
	EMVBase_Trace("PURE-info: SDAHash", SDAHash);
    ...
    if(DOLDataitem != NULL)
    {
        if(DOLDataitem->len)
        {
			...
			EMVBase_TraceHex("PURE-info: input pdol data", DOLDataitem->data, DOLDataitem->len);
    ...
    if(DOLDataitem != NULL)
    {
        if(DOLDataitem->len)
        {
			...
			EMVBase_TraceHex("PURE-info: input cdol1 data", DOLDataitem->data, DOLDataitem->len);
            index += DOLDataitem->len;
        }
    }
	...
	EMVBase_TraceHex("PURE-info: input gac ret data", tempApp_UnionStruct->EMVTradeParam->GenerateACRetData, tempApp_UnionStruct->EMVTradeParam->GenerateACRetDataLen);
...
unsigned char pure_GAC1AndAnalys(PURETradeUnionStruct *tempApp_UnionStruct, unsigned char TermAnaResult)
{
	...
	if(0x00 != tempApp_UnionStruct->EMVTradeParam->OfflineCAMSelectedIndicator)//ODA Select CDA/DDA/SDA,goto recover and check CAPK,IPK,ICCPK
	{
		...
		else
		{
			...
			{
				tempApp_UnionStruct->EMVTradeParam->RecoverPKSuccess = 1;
			}
		}

		if(ODAFail)
		{
			EMVBase_Trace("PURE-info: ODA fail because Recover PK error\r\n");
...
//		return RLT_EMV_OFFLINE_DECLINE;
	...
	EMVBase_Trace("pure-info: Impletion: %02X\r\n", Impletion);
	EMVBase_Trace("pure-info: Kernel Cap: %02X\r\n", KernelCap[2]);

	EMVBase_Trace("pure-info: and result: %d\r\n", (KernelCap[2] & 0x10));

	if(((PURE_TRANS_APPAUTHTRANS == TransType) && (Impletion & 0x10)) || (0x10 != (CryptInfo & 0xC0)) || (0 == (KernelCap[2] & 0x10)))
	{
...
unsigned char pure_TransProcessWithNoOutcome(PURETradeUnionStruct *tempApp_UnionStruct)
{
	unsigned char TransType = tempApp_UnionStruct->EMVTradeParam->CurTransType;
	unsigned char Impletion = tempApp_UnionStruct->EMVTradeParam->PureImplementationOption;
	unsigned char KernelCap[5];
...
unsigned char pure_TransProcess(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	{
		return pure_Outcome_FinancialTransactionCompleted_TransactionDeclined(tempApp_UnionStruct, 1);
	}
	else if(0x80 == (CID & 0xC0))	//ARQC
	{
...
{
    unsigned char SSAHash[20], Tag[3]={0};
    unsigned short index = 0;
	...
	EMVBASETAGCVLITEM *item_taglist = NULL;
	EMVBASETAGCVLITEM *item_tag = NULL;
    unsigned char *SSAToSign = NULL;
	...
	memset(SSAToSign, 0, 2048 + 512);
	if(SSAToSign == NULL)
	{
		EMVBase_Trace("pure-info: emv malloc error!\r\n");
        return RLT_EMV_ERR;
	}
	...
	EMVBase_TraceHex("input hash data 1", SSAToSign, index);

    emvbase_free(SSAData);

    if((index + tempApp_UnionStruct->EMVTradeParam->AuthDataLen) > (2048 + 512))
    {
		EMVBase_Trace("pure-info: hash input data len over limit\r\n");
	...
	EMVBase_TraceHex("SDVerify-AuthData: ", tempApp_UnionStruct->EMVTradeParam->AuthData, tempApp_UnionStruct->EMVTradeParam->AuthDataLen);

	EMVBase_Trace("pure-info: SSAToSign = %p\r\n", SSAToSign);
	memcpy(SSAToSign+index, tempApp_UnionStruct->EMVTradeParam->AuthData, tempApp_UnionStruct->EMVTradeParam->AuthDataLen);
    index += tempApp_UnionStruct->EMVTradeParam->AuthDataLen;

	EMVBase_TraceHex("input hash data 2", SSAToSign, index);

	EMVBase_TraceHex("pure-info: before memcpy Tag", Tag, 3);

	if(emvbase_avl_checkiftagexist(EMVTAG_SDATagList))
	{
		EMVBase_Trace("pure-info: flag2\r\n");
		item_taglist = emvbase_avl_gettagitempointer(EMVTAG_SDATagList);
		EMVBase_Trace("pure-info: flag3\r\n");

		if(item_taglist && item_taglist->len == 1)	//9F4A contain only 82
		{
			EMVBase_Trace("pure-info: flag4\r\n");
			memcpy(Tag, item_taglist->data, item_taglist->len);
			EMVBase_Trace("pure-info: flag5\r\n");
			EMVBase_TraceHex("pure-info: Tag in 9F4A", Tag, 3);
			item_tag = emvbase_avl_gettagitempointer(Tag);
			EMVBase_Trace("pure-info: item_tag = %p\r\n", item_tag);
			EMVBase_Trace("pure-info: flag6\r\n");
			EMVBase_TraceHex("pure-info: item_tag->data", item_tag->data, item_tag->len);
			memcpy(SSAToSign+index, item_tag->data, item_tag->len);
			EMVBase_Trace("pure-info: flag7\r\n");
			index += item_tag->len;
		}
	}

	EMVBase_TraceHex("input hash data", SSAToSign, index);

	EMVBase_Hash_Back(SSAToSign, index, SSAHash);

	EMVBase_TraceHex("Recover Hash", recovSSA->HashResult, 20);
	EMVBase_TraceHex("Calculate Hash", SSAHash, 20);

    if(recovSSA->HashInd == 0x01)  //SHA-1 algorithm
    {
        if(memcmp(recovSSA->HashResult, SSAHash, 20))	//only for FIME Tool
        {
			EMVBase_Trace("pure-info: SDAD hash different\r\n");
            emvbase_free(SSAToSign);
            emvbase_free(recovSSA);
            return RLT_EMV_ERR;
        }
...
unsigned char pure_DataAuth_SDA(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	unsigned char Implementation = tempApp_UnionStruct->EMVTradeParam->PureImplementationOption;
	unsigned char TransType = tempApp_UnionStruct->EMVTradeParam->CurTransType;

	emvbase_avl_setvalue_or(EMVTAG_TVR, 0, 0x02);
	//SDA success when SDA Tag List is missing
	...
	{
		EMVBase_Trace("pure-info: SDA fail because SDA Tag List missing or Auth Data invalid\r\n");
		ODAFail = 1;
	}
	else
	{
...
//		}
		...
		if(!tempApp_UnionStruct->EMVTradeParam->ODADataChecked)
		{
			if(RLT_EMV_OK != pure_CheckDataMissSDA(tempApp_UnionStruct))
			{
				emvbase_avl_setvalue_or(EMVTAG_TVR, 0, 0x20);		//ICC data missing
				EMVBase_Trace("pure-info: SDA fail because miss mandatory data\r\n");
				ODAFail = 1;
			}
		}

		if(!tempApp_UnionStruct->EMVTradeParam->RecoverPKSuccess)
		{
//			if(tempApp_UnionStruct->EMVTradeParam->IPKModulLen == 0)
//			{
//				if(RLT_EMV_OK != pure_GetIPK(tempApp_UnionStruct))	//retrieval of the issuer public key modulus
//				{
					EMVBase_Trace("pure-info: SDA fail because Recover PK error\r\n");
					ODAFail = 1;
//				}
//			}
		}
		...
		{
			EMVBase_Trace("pure-info: SDA fail because verify Auth Data error\r\n");
			ODAFail = 1;
		}
	}
...
unsigned char pure_DataAuth_CDA(PURETradeUnionStruct * tempApp_UnionStruct)
{
	...
	else
	{
		if(!tempApp_UnionStruct->EMVTradeParam->ODADataChecked)
		{
			retCode = pure_CheckDataMissDDA(tempApp_UnionStruct);
			EMVBase_Trace("pure-info: pure_CheckDataMissDDA ret = %d\r\n", retCode);
			if(retCode != RLT_EMV_OK)
			{
				emvbase_avl_setvalue_or(EMVTAG_TVR, 0, 0x20);		//ICC data missing
				ODAFail = 1;
...
unsigned char pure_DataAuth(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	EMVBase_Trace("pure-info: ODAIndicator = %02X\r\n", ODAIndicator);
	...
	else if(0x02 == ODAIndicator)	//CDA
	{
		...
		if(RLT_EMV_OK != retCode)
		{
			EMVBase_Trace("pure-error: CDA fail\r\n");
			emvbase_avl_setvalue_and(EMVTAG_CryptInfo, 0, 0x3F);	//CID set to ‘00’ – AAC
		}
...
unsigned char pure_CardHolderVerf_onlinepin(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	static unsigned char callbackFlag = 0;
#if 1
	tempApp_UnionStruct->EMVTradeParam->RequestOnlinePIN = 1;
	return RLT_EMV_OK;
#else
...
unsigned char pure_CardHolderVerf(PURETradeUnionStruct *tempApp_UnionStruct)
{
	...
	if(tempApp_UnionStruct->EMVTradeParam->CCIDExistInGPO)
	{
		...
		if((TTPI[1] & 0x40) || (0x00 !=(CCID & 0x07)))	//ICC or Terminal request CVM
		{
			...
			{
				EMVBase_Trace("pure-info: Card Request No CVM\r\n");
			...
			{
				EMVBase_Trace("pure-info: Card Request Online PIN\r\n");
				...
				if(TTPI[0] & 0x04)
				{
					...
					emvbase_avl_createsettagvalue(EMVTAG_CVMResult, CVMRes, 3);
					emvbase_avl_setvalue_or(EMVTAG_TVR, 2, 0x04);
					pure_CardHolderVerf_onlinepin(tempApp_UnionStruct);
				}
				else
				{
					CVMRes[2] =CVMR_FAIL;
				...
				tempApp_UnionStruct->EMVTradeParam->PureCVMParameter = SDK_OUTCOME_CVM_ONLINEPIN;
				EMVBase_Trace("pure-info: Set CVM = %02x\r\n", tempApp_UnionStruct->EMVTradeParam->PureCVMParameter);
			}
			else if(0x01 == (CCID & 0x07))	//Signature
			{
				EMVBase_Trace("pure-info: Card Request Signature\r\n");
			...
			{
				EMVBase_Trace("pure-info: Card Request CDCVM\r\n");
			...
			{
				EMVBase_Trace("pure-info: Card Request using 8E\r\n");
